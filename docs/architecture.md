...
currently composes `health`, `auth`, `users`, `conversations`, `messages`, and `ai` features on top of
...
- `app.ts` mounts the API rate limiter once, then the auth feature's public
  router, the authentication middleware, and the protected auth/users/conversations/messages/AI
